...
from .pycocotools_caod.cocoeval_wrappers import COCOEvalXclassWrapper
import ipdb
# from .pycocotools_caod.cocoeval import COCOeval
...
def screenout_ignored_labels(gts,detections):
    #moving the image without testing class

    original_length = len(gts)
    empty_list = []
    for ind in range(original_length):
        save_flag = True
        for item in gts[ind]:
            if item[6] != 1:
                save_flag = False
    ...
    gts = [gts[i] for i in range(len(gts)) if i not in empty_list]
    detections = [detections[i] for i in range(len(detections)) if i not in empty_list]

    return gts, detections

def screenout_unseen_labels(gts):
    #moving the image with unseen class
    original_length = len(gts)
    for ind in range(original_length):
        moving_idx = []
        for idx, item in enumerate(gts[ind]):
            if item[6] == 1:
                moving_idx.append(idx)
        gts[ind] = np.delete(gts[ind], moving_idx)
    return gts

def set_ignored_to_0(gts):
    modified_list = []
    for item in gts:
        modified_item = item.copy()
        modified_item['ignored_split'] = 0 # Setting the ignored_split column to 1
        modified_list.append(modified_item)
    return modified_list

def evaluate_detection(gt_boxes_list, dt_boxes_list, classes=("car", "pedestrian"), height=240, width=304,
                       time_tol=50000, return_aps: bool = True, ignored: bool = True):
    """
    ...
    return _coco_eval(flattened_gt, flattened_dt, height, width, labelmap=classes, return_aps=return_aps, ignored = ignored)
...
def  _coco_eval(gts, detections, height, width, labelmap=("car", "pedestrian"), return_aps: bool = True, ignored: bool = True):
    ...
    out_keys = ('AP', 'AP_50', 'AP_75', 'AP_S', 'AP_M', 'AP_L','AR_10','AR_20','AR_30','AR_50','AR_100','AR_300')
    ...
    #screen out the images without the unignored labels:
    gts,detections = screenout_ignored_labels(gts,detections)

    # gts = screenout_unseen_labels(gts)
    
    if ignored == False:
        gts = set_ignored_to_0(gts)
...
def coco_eval_return_metrics(coco_eval: COCOEvalXclassWrapper):
    pass
...
def _to_coco_format(gts, detections, categories, height=240, width=304):
    ...
    for image_id, (gt, pred) in enumerate(zip(gts, detections)):
        ...
        for bbox in gt:
            ...
            w, h = bbox['w'], bbox['h']
            area = w * h
            ignored_split = bbox['ignored_split']
            annotation = {
                "area": float(area),
                "iscrowd": False,
                "image_id": im_id,
