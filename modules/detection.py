    ...
    def training_step(self, batch: Any, batch_idx: int) -> STEP_OUTPUT:
        ...
        for tidx in range(sequence_len):
            ...
            if len(current_labels) > 0:
                ...
                if len(prev_reg) > 0:
                    prev_reg = prev_reg[valid_batch_indices]
                prev_first_sample_flag = is_first_sample[valid_batch_indices]
                if first_valid_flag == False:
                    prev_first_sample_flag = [False for _ in prev_first_sample_flag]
                prev_reg_list = [prev_first_sample_flag, prev_reg]
            ...
            if len(current_labels) > 0 and len(inference_valid) > 0:
                # prev_reg = torch.cat([predictions, predictions_rest],dim=0)
                ...
                prev_reg = predictions
            elif len(current_labels) == 0 and len(inference_valid) > 0:
                prev_reg = predictions_rest

            first_valid_flag = False

            prev_reg = prev_reg[:,:,0:4]
        ...
        losses = self.merge_dicts_and_average(losses_list)
