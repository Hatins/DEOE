...
import os
import numpy as np
from scripts.genx.tools import dat_events_tools as dat
from scripts.genx.tools import npy_events_tools as npy_format
...
class PSEELoader(object):
    ...
    def load_n_events(self, ev_count):
        ...
        if ev_count >= count:
            ...
            result = np.int64(pos) + np.int64(ev_count) * np.int64(self._ev_size)
            self._file.seek(result)
