...
def get_base_delta_ts_for_labels_us(unique_label_ts_us: np.ndarray, dataset_type: str = 'gen1') -> int:
    ...
    # np.diff 用后一个元素减去前一个元素，这里是计算的间隔时间
    diff_us = np.diff(unique_label_ts_us)

    #np.median 求一段序列中的中位数
...
class EventRepresentationFactory(ABC):
    ...
    #abstractmethod: 只有子类实现了该抽象方法才能被实例化
    #property: 只读属性，在调用时后面不需要接()
    @property
    @abstractmethod
    def name(self) -> str:
        ...
...
if __name__ == '__main__':
    ...
    parser.add_argument('-input_dir', default='/data/zht/establish_frame/gen4/gen4_test_h5_v1')
    parser.add_argument('-target_dir', default='/data/zht/establish_frame/gen4/gen4_test_process_h5_v1')
    ...
    parser.add_argument('-ds', '--dataset', default='gen4', help='gen1 or gen4')
    parser.add_argument('-np', '--num_processes', type=int, default=20, help="Num proceesses to run in parallel")
