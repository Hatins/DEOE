...
class S5SSM(torch.nn.Module):
    def __init__(
        ...
        """
        super().__init__()
        self.Lambda = torch.nn.Parameter(torch.view_as_real(lambdaInit)) #the A

        self.degree = degree
        self.liquid = liquid

        self.bcInit = bcInit #the way to get B and C

        self.bidir = bidir
        self.bandlimit = bandlimit #a parameter
