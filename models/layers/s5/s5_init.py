...
def make_HiPPO(N):
    ...
    """
    '''
    (A_LegS)nk = - [(2n+1)^(1/2)] * [(2k+1)^(1/2)], n > k
                   [n + 1],                         n = k
                   0,                               n < k
    '''
...
def make_DPLR_HiPPO(N):
    ...
    '''
    NPLR representation of HiPPO: A_LegS = (A_HiPPO)^(Normal) - P(Q^T) 
    (A_LegS)nk = - [(2n+1)^(1/2)] * [(2k+1)^(1/2)], n > k
                   [n + 1],                         n = k
                   0,                               n < k

    B_n = (2n+1)^(1/2)   P_n = (n + 1/2)^(1/2)
    '''
    A, P, B = make_NPLR_HiPPO(N) 
    '''
    DPLR representation of HiPPO: A_LegS = (A_HiPPO)^(Normal) - P(P)^T
    '''
    S = A + P[:, np.newaxis] * P[np.newaxis, :] #(A_LegS)^Normal = A_LegS + P_Legs * (P_Legs)^T

    S_diag = np.diagonal(S)  #(A_LegS)^Normal = VΛV*
    ...
    # Λ = Lambda_real + 1j * Lambda_imag; V = V
