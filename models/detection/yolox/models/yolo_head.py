...
class YOLOXHead(nn.Module):
    ...
    def get_losses(
        ...
        if torch.sum(prev_reg_valid)>0 and torch.sum(~prev_reg_valid)> 0:
            ...
            TT = (T_top_indices / 5040).int()
            T_odd = T_top_indices - 5040*TT
            T_final_indices = T_odd + T_indices[TT]*5040
       

            F_indices = (torch.nonzero(~prev_reg_valid).squeeze()).to(outputs.device)
            ...
            FF = (F_top_indices / 5040).int()
            F_odd = F_top_indices - 5040 * FF
            F_final_indices = F_odd + F_indices[FF]*5040
        ...
        #TODO:hyperparameters-4
   

        # top_indices = self.find_topk_indices(scores, obj_preds.shape[0], self.average_img_num)
        ...
        # if torch.sum(matched_final_score_mask) > 0:
        ...
        # loss_obj = (
        #     self.bcewithlog_loss(obj_preds.view(-1, 1), obj_targets_clone) * obj_weight
        # ).sum() / num_all

        loss_iou_boost = self.iou_loss_boost(IoU_boost)
        # print('the len of item is {}, other is {}'.format((obj_targets_clone.sum()) - (obj_targets.bool().sum()), loss_iou_boost))
        loss_iou = (
