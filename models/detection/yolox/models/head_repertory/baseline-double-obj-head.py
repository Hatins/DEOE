...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin, labels=None):
        ...
        for k, (obj_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            obj_feat_boost = obj_conv(obj_x_boost)
            obj_output_boost = self.obj_preds_boost[k](obj_feat_boost)
            ...
            if self.training:
                output = torch.cat([reg_output, obj_output, obj_output_boost], 1)
            ...
            inference_output = torch.cat(
                [reg_output, obj_output.sigmoid() * obj_output_boost.sigmoid()], 1
            )
        ...
        if self.training:
            ...
            losses = {
                "loss": losses[0],
                "iou_loss": losses[1],
                "obj_loss": losses[2], # object-ness
                "obj_loss_boost":losses[3],
                "l1_loss": losses[4],
                "num_fg": losses[5],
            }
    ...
    def get_losses(
        ...
        obj_preds_boost = outputs[:, :, 5:6]  # [batch, n_anchors_all, 1]
        ...
        loss = reg_weight * loss_iou + loss_obj + loss_l1 + loss_obj_boost
